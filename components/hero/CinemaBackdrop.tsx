...
/**
 * O vídeo de fundo persistente.
 *
 * Fica `fixed` atrás do Hero E da seção "Escolha o seu caminho" — é isso que produz a
 * sensação pedida no §7: o vídeo não terminou, ele continua existindo atrás da página.
 * Conforme o scroll avança ele escurece, desfoca e amplia; depois da seção 02 sai de cena
 * por opacidade, mas o elemento NÃO desmonta, senão o áudio morreria junto.
 */
export function CinemaBackdrop({ dict }: { dict: Dictionary }) {
  ...
  const scale = useTransform(scrollY, [0, vh * 2], [1.04, 1.16]);
  ...
  useEffect(() => {
    ...
    void video.play().catch(() => {
      // Autoplay bloqueado mesmo mudo: o poster continua no lugar, sem erro no console.
    });
  }, [wantsVideo]);
  ...
  return (
    <>
      <motion.div
        aria-hidden
        className="grain vignette pointer-events-none fixed inset-0 z-0 overflow-hidden bg-midnight-deep"
        style={{ opacity }}
      >
        ...
        {/* Rampa escura de cima para baixo: garante contraste do texto sobre qualquer frame. */}
